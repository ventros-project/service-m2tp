...
function read(request, response) {
  const deviceClass = request.params["class"];

  const deviceAddress = m2tp.m2tp_findMemberAddress(deviceClass);
  if (!deviceAddress) {
    response.status(404).send();
    return;
  }

  const lastData = receiveStation.get(deviceAddress);
  if (lastData) {
...
function write(request, response) {
  ...
  m2tp.m2tp_startSend(deviceAddress);
